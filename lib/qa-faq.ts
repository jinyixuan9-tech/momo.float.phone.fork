...
### Q: 怎么自定义聊天状态栏/心声的样式？
聊天信息设置（聊天室右上角进入）→ 关闭「原生状态栏」（状态值 [好感度] 等与内心 [内心] 的默认输出会整块从提示词移除，好感度数值随之停更）→ 点「自定义状态栏」：填两样东西——输出契约（即提示词「状态栏」章节的整段正文，含逻辑、格式与 [状态栏] 包裹要求，预填了微博主页模板可照改）和输出渲染（一段 HTML+CSS+JS 模具，沙盒运行，数据经 window.STATUS_RAW 注入），弹窗里可用示例数据实时预览。AI 每轮只输出数据，画法固定省 token。仅默认预设支持（或在自己预设里写入 {{statusRegionSection}} 宏的预设）；已用正则渲染状态栏的会话请二选一，两套同开会互相竞争。切回原生随时可逆，旧消息永远按生成时的模式渲染。

### Q: MCP 填本机/内网地址提示「不允许代理访问本机或内网地址」？
默认 MCP 请求走官网服务端代理（绕浏览器跨域），代理到不了你电脑上的 localhost/内网，也会被安全防线拦截。解法：编辑该 MCP 服务器，勾选「直连模式」——请求改由浏览器直接发起，本机/局域网天然可达。要求：MCP 服务器允许跨域（CORS，Access-Control-Allow-Origin），且为 Streamable HTTP 传输（地址通常以 /mcp 结尾；/sse 的旧式 SSE 传输不支持直连）。iOS Safari 对 localhost 的支持不如电脑端 Chrome/Edge 稳。另一条路：用 cloudflared/ngrok 之类隧道把本地服务暴露成公网 https 地址，不勾直连也能用。
...
### Q: 生图 / 语音 / 在线音乐怎么启用？
都是可选增强：生图在设置里配置生图 API 或代理；语音合成用 Minimax（自配 key）；在线音乐需自行部署 NeteaseCloudMusicApi 兼容实例并在音乐 APP 设置里填地址（或部署时配 NEXT_PUBLIC_DEFAULT_NETEASE_API_BASE）。
