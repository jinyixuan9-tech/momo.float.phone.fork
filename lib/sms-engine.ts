...
export async function generateSmsReply(thread: SmsThread, state: SmsState, mode: "reply" | "proactive" = "reply"): Promise<{ messages: { original: string; translated: string }[]; awareness?: SmsThread["awareness"]; suspicion?: string; block?: boolean; summary?: string; channel?: "sms" | "request" | "wait"; requestMessage?: string }> {
  ...
  const history = state.messages.filter(m => m.threadId === thread.id && m.delivered).slice(-16).map(m => `${m.direction === "outgoing" ? "来信" : "我"}: ${compact(m.original, 180)}`).join("\n");
  ...
  const instruction: LLMMessage = { role: "system", content: [
    ...
    mode === "proactive" ? "这是一次可选的主动联系机会。若 Chat 被拉黑，可在短信、重新发 Chat 好友申请、等待之间选择一个；已有待处理申请时不要重复申请。不要同一轮跨两处同时联系。" : "仅回应当前来信；可以分成 1 到 3 条短信。",
    "只输出 JSON：{\"send\":true/false,\"channel\":\"sms/request/wait\",\"requestMessage\":\"申请附言，选择 request 时填写\",\"messages\":[{\"original\":\"角色语言原文\",\"translated\":\"忠实简体中文译文\"}],\"awareness\":\"unknown/suspected/confirmed\",\"suspicion\":\"角色可解释的线索\",\"block\":false,\"summary\":\"仅基于自己已知事实的短信关系摘要，200字以内\"}。identity 的确认必须有真实可见的充分证据；不可仅因发送者实际是用户而确认。不要在短信里写动作或系统事实。",
  ].join("\n") };
  const raw = await sendLLMRequest(api, preset, [...prompt, instruction, { role: "user", content: mode === "reply" ? "收到短信后决定是否回应。" : "现在是否想主动给用户真实号码发短信？" }], regexes, { characterName: character.name, userName: real ? resolveUserIdentity(character.id, "chat")?.name : "陌生号码" }, { appId: "sms", appTags: ["sms"], skipOutputRegex: true });
  const match = raw.match(/\{[\s\S]*\}/);
  if (!match) throw new Error("短信回复格式无效，可重试");
  const data = JSON.parse(match[0]) as Record<string, unknown>;
  const messages = data.send === false ? [] : (Array.isArray(data.messages) ? data.messages : []).slice(0, 3).map((x: unknown) => { const row = x as Record<string, unknown>; return { original: String(row?.original ?? "").trim().slice(0, 800), translated: String(row?.translated ?? "").trim().slice(0, 800) }; }).filter(x => x.original);
