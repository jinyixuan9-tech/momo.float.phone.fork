...
type Card = { original: string; translated: string };
type Event = { kind: "birthday"; year: number } | { kind: "anniversary"; days: number };
...
export function LysnCalendar({ state, characterId, artistName, avatar, onDateChange, onBirthday }: {
  ...
  onBirthday: (year: number) => Promise<Card>;
}) {
  const today = localDay(new Date());
  const [month, setMonth] = useState(() => new Date(new Date().getFullYear(), new Date().getMonth(), 1));
  const [selected, setSelected] = useState(today);
  const [event, setEvent] = useState<Event | null>(null);
  const [letter, setLetter] = useState<Card | null>(null);
  ...
  return <div className={styles.calendar}>
    ...
    {event && <div className={styles.backdrop} onMouseDown={e => { if (e.target === e.currentTarget) setEvent(null); }}><section role="dialog" aria-modal="true" aria-label={event.kind === "birthday" ? "生日留言" : "订阅纪念"} className={event.kind === "birthday" ? styles.birthday : styles.anniversary}><button type="button" aria-label="关闭卡片" className={styles.close} onClick={() => setEvent(null)}><X size={21}/></button>{event.kind === "birthday" ? <><div className={styles.portrait}>{portraitUrl ? <img src={portraitUrl} alt=""/> : artistName.slice(0, 1)}</div><div className={styles.from}><i>from.</i> {artistName}</div><div className={styles.letter}><span>✦ &nbsp; Birthday note &nbsp; ✦</span>{loading ? <p>正在写生日留言…</p> : error ? <button type="button" onClick={() => openEvent(event)}>{error} · 重试</button> : <><p>{letter?.original || ""}</p><small>{letter?.translated || ""}</small></>}<img className={styles.cake} src="/lysn-assets/birthday-cake.webp" alt="粉色蝴蝶结生日蛋糕"/></div></> : <><div className={styles.anniversaryTitle}>♥ &nbsp; HAPPY &nbsp; ♥<strong>bubble</strong><span>ANNIVERSARY</span></div><div className={styles.cartoonNumber}>{event.days}<span>DAYS</span></div><img className={styles.gifts} src="/lysn-assets/anniversary-gifts.webp" alt="一堆彩色礼物"/><p>我们在 Bubble 的第 {event.days} 天</p><div className={styles.confetti}>♥　✦　♥　✳</div></>}</section></div>}
  </div>;
}
