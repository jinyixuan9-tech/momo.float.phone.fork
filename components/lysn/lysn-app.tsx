...
type TranslationTarget = "zh-CN" | "zh-TW" | "yue" | "en" | "ja" | "ko" | "th" | "off";

const INPUT_LANGUAGE_OPTIONS: Array<{ code: TranslationTarget; label: string; buttonLabel: string }> = [
  { code: "zh-CN", label: "中文(简体)", buttonLabel: "中文" },
  { code: "zh-TW", label: "中文(繁体)", buttonLabel: "繁体" },
  { code: "yue", label: "粤语", buttonLabel: "粤语" },
  { code: "en", label: "英语", buttonLabel: "英语" },
  { code: "ja", label: "日语", buttonLabel: "日语" },
  { code: "ko", label: "韩语", buttonLabel: "韩语" },
  { code: "th", label: "泰语", buttonLabel: "泰语" },
  { code: "off", label: "关闭翻译", buttonLabel: "关闭翻译" },
];
...
async function requestTranslation(url: string, apiKey: string, text: string, target: TranslationTarget) {
  const payload = {
    text,
    q: text,
    content: text,
  ...
  if (!response.ok) throw new Error(raw || `翻译失败（${response.status}）`);
  try {
    const parsed = JSON.parse(raw) as any;
    const candidate = parsed?.translatedText ?? parsed?.translation ?? parsed?.translated ?? parsed?.result ?? parsed?.text ?? parsed?.data?.translatedText ?? parsed?.data?.translation ?? parsed?.data?.translated ?? parsed?.data?.result ?? parsed?.data?.text ?? parsed?.choices?.[0]?.message?.content;
...
export function LysnApp({ onClose, onNotice }: { onClose: () => void; onNotice?: (text: string) => void }) {
  ...
  const [inputTranslateOpen, setInputTranslateOpen] = useState(false);
  const [inputTranslateTarget, setInputTranslateTarget] = useState<TranslationTarget>("ko");
  const [inputTranslationPreview, setInputTranslationPreview] = useState("");
  const [draftTranslationSource, setDraftTranslationSource] = useState<{ source: string; translated: string } | null>(null);
  const [inputTranslationLoading, setInputTranslationLoading] = useState(false);
  const [inputLanguageSheetOpen, setInputLanguageSheetOpen] = useState(false);
  ...
  const inputTargetLabel = useMemo(() => INPUT_LANGUAGE_OPTIONS.find(item => item.code === inputTranslateTarget)?.buttonLabel || "韩语", [inputTranslateTarget]);
  ...
  const translateDraft = async (nextTarget = inputTranslateTarget) => {
    if (!draft.trim()) {
      setInputTranslationPreview("");
      setInputTranslateOpen(true);
      return;
    }
    if (nextTarget === "off") {
      setInputTranslationPreview("");
      setInputTranslateOpen(false);
      return;
    }
    if (!state.settings.translationApiUrl) {
      onNotice?.("请先在设置中填写翻译 API URL");
      return;
    }
    setInputTranslationLoading(true);
    setInputTranslateOpen(true);
    try {
      const translated = await requestTranslation(state.settings.translationApiUrl, state.settings.translationApiKey, draft, nextTarget);
      setInputTranslationPreview(translated);
    } catch (error) {
      onNotice?.(error instanceof Error ? error.message : "翻译失败");
    } finally {
      setInputTranslationLoading(false);
    }
  };

  const useTranslatedDraft = () => {
    if (!inputTranslationPreview.trim()) return;
    setDraft(inputTranslationPreview);
    setDraftTranslationSource({ source: draft.trim(), translated: inputTranslationPreview.trim() });
    setInputTranslateOpen(false);
    onNotice?.("已替换为翻译文本");
  };
  ...
  const sendFan = (): boolean => {
    ...
    appendLysn(selected, [{ sender: "fan", kind: "text", original: text, translated: text, ...(draftTranslationSource?.translated === text ? { sourceText: draftTranslationSource.source } : {}) }]);
    setDraft("");
    setDraftTranslationSource(null);
    setInputTranslationPreview("");
    setInputTranslateOpen(false);
    return true;
  };
  ...
  const back = () => {
    if (inputLanguageSheetOpen) { setInputLanguageSheetOpen(false); return; }
  ...
  const renderMessage = (item: LysnMessage | LysnMessage[]) => {
    ...
    const fanTranslated = m.sender === "fan" && Boolean(m.sourceText);
      ...
      >
        ...
        <div className={styles.messageBody}>
          ...
          <div className={`${styles.bubble} ${m.kind === "voice" ? styles.voiceBubbleShell : ""}`} onClick={bubbleClick}>
            ...
            {fanTranslated && <small className={styles.fanSourceText}>{m.sourceText}</small>}
  ...
  return (
    <div className={styles.app} data-root={root ? "true" : undefined}>
      ...
      {inputLanguageSheetOpen && <div className={styles.overlay} onMouseDown={e => { if (e.target === e.currentTarget) setInputLanguageSheetOpen(false); }}><section className={`${styles.sheet} ${styles.languageSheet}`} role="dialog" aria-label="对话框翻译目标语言"><header><b>对话框翻译目标语言</b><button type="button" onClick={() => setInputLanguageSheetOpen(false)} aria-label="关闭"><X size={20}/></button></header><div className={styles.languageList}>{INPUT_LANGUAGE_OPTIONS.map(item => <button type="button" key={item.code} className={styles.languageItem} onClick={() => { setInputTranslateTarget(item.code); setInputLanguageSheetOpen(false); if (item.code === "off") { setInputTranslateOpen(false); setInputTranslationPreview(""); } else { void translateDraft(item.code); } }}><span>{item.label}</span>{inputTranslateTarget === item.code && item.code !== "off" ? <i><Check size={16}/></i> : <em/>}</button>)}</div></section></div>}
