...
export function LysnApp({ onClose, onNotice }: { onClose: () => void; onNotice?: (text: string) => void }) {
  ...
  const createArtistMessage = async (id: string, mode: "new" | "reply", fanText?: string) => {
    ...
    try {
      ...
      for (const item of [generated, ...(generated.extra || [])]) {
        ...
        rows.push({ sender: "artist", kind, original: item.original, translated: item.translated, imageUrl, photoId });
      }
  ...
  return <div className={styles.app} data-font-size={state.settings.fontSize}>
    ...
    }) : <div className={styles.empty}>还没有消息。可以通过菜单生成第一条艺人消息。</div>}<div ref={endRef}/></main><form className={styles.composer} onSubmit={e => { e.preventDefault(); void summonArtist(); }}><textarea rows={1} aria-label="回复艺人" placeholder="输入消息，回车发送…" value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); sendFan(); } }} /><button type="submit" disabled={busy} aria-label="召唤艺人回复" title="召唤艺人回复"><Send size={21}/></button></form></>}
    ...
    {menuOpen && route === "chat" && character && <div className={styles.overlay} onMouseDown={e => { if (e.target === e.currentTarget) setMenuOpen(false); }}><section className={styles.sheet}><header><b>{displayName}</b><button type="button" onClick={() => setMenuOpen(false)} aria-label="关闭"><X size={19}/></button></header><button type="button" onClick={() => { setMenuOpen(false); setRoute("artist"); }}>查看艺人资料</button><button type="button" onClick={startArtistEdit}>编辑 LYSN 资料</button><button type="button" onClick={() => { setSettingsReturn("chat"); setMenuOpen(false); setRoute("settings"); }}>设置</button><button type="button" disabled={busy} onClick={() => { setMenuOpen(false); void summonArtist(); }}>{busy ? <LoaderCircle size={17} className={styles.spin}/> : <Sparkles size={17}/>}召唤艺人消息</button><button type="button" className={styles.danger} onClick={() => { subscribe(character.id); setMenuOpen(false); }}>取消订阅</button></section></div>}
  </div>;
}
