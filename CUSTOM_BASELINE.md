...
## v0.5.2 · WVS LIVE Playback / Artist Watch / Collaboration
- 删除竖屏 LIVE 模板，普通文字 LIVE 统一使用横屏 16:9 Stage；观看页顶部收成真实 WVS 感的黑色 `● LIVE / REPLAY` 状态栏，不保留分享按钮。
- 修复 v0.5.1 懒播放时序：AI 生成完成后才开始安排 segment / comment 的显现时间，避免模型响应耗时导致进入直播间时整轮内容已经一次性出现。主播内容逐段播放，粉丝评论逐条出现。
- 直播底栏统一为输入框 + 小飞机 + 爱心：回车只发送评论、不触发 AI；小飞机在有新评论时把本轮评论一起召唤给角色，无新评论时等同“继续播放”；爱心仅做可重复点击的飘心 UI，并轻量增加本场点赞数。
- Live Stage 保留角色 WVS ID 作为发言归属，不显示头像；action 继续仅在发生新动作时出现，并以斜体的省略主语现场描写显示。
- 成员资料新增“默认 LIVE 封面”；新个人 LIVE 默认复用该角色封面，也可在开播前为单场覆盖。回放卡改为封面 + 时长 + 账号/标题 + 播放/点赞/聊天数据，并支持单场换封面与“删除该回放”。
